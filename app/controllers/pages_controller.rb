class PagesController < ApplicationController
  ...
  def analysis    
    ...
    param_array = si_pool.map { |si| [si.net_stock_issues, si.rel_accruals, si.net_op_assets_scaled, si.assets_growth, si.invest_to_assets, si.adj_invest_to_assets, si.l_52_wk_price, si.profit_prem, si.roa_q, si.dist_total_2] }
    ...
    ts_array = @sis.map { |si| si[6] }.sort
    @sis.each do |si| 
      si[7] = (ts_array.index(si[6]) + 1)/ts_array.length.to_f
      if si[7] >= 0.9
        si[8] = si[18] > 7 ? "Yes" : "No"
      elsif si[7] <= 0.1
        si[8] = si[18] > 8 ? "Yes" : "No"
